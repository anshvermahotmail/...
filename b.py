...
ps_result = subprocess.run([ps_cmd, "-Command", powershell_script], capture_output=True, text=True)
try:
    import json
    ps_data = json.loads(ps_result.stdout.replace('\n', '').strip())
except:
    print("❌ PowerShell output parsing failed. Raw output:")
