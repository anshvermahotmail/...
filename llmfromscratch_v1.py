...
def forward(x, W_embed, W_q, W_k, W_v, W_proj):
    """
    Forward pass: Embedding -> Self-Attention -> Logits
    """
    x_embed = W_embed[x]  # Shape: [block_size, embed_size]
    context = self_attention(x_embed, W_q, W_k, W_v, W_proj)
    logits = context.sum(axis=0)  # Aggregate
...
def train(data, stoi, W_embed, W_q, W_k, W_v, W_proj, epochs=500, lr=0.01):
    """
    Simple training loop with random subsequences.
    We simulate learning by updating W_proj (simplified).
    """
    for epoch in range(epochs):
        ...
        grad = probs
        grad[target] -= 1  # dL/dlogit approx
        grad = grad.reshape(1, -1)

        # Update projection weights (toy gradient step)
        W_proj -= lr * (W_proj - grad.T[:W_proj.shape[0], 0][:, None])
...
def respond_to_query(name, corpus, stoi, itos):
    ...
    tokens = [stoi.get(ch, 0) for ch in prompt[-block_size:]]
    probs = forward(tokens, W_embed, W_q, W_k, W_v, W_proj)
    top_indices = np.argsort(probs)[-40:]  # top likely characters
