...
FASTAPI_URL = f"http://127.0.0.1:{FASTAPI_PORT}/ask"
...
async def ask_question(req: Request):
    ...
    if re.match(r"^[\d\s\+\-\*\/\.\(\)]+$", question):
        try:
            ...
            debug_logs.append(f"Detected math expression: {question}")
            debug_logs.append(f"Evaluated result: {result}")
            return {"answer": f"🧮 Result: {result}", "log": debug_logs}
        except Exception as e:
            debug_logs.append(f"Math evaluation failed: {e}")
...
def run_fastapi():
    uvicorn.run(app, host="127.0.0.1", port=FASTAPI_PORT, log_level="debug")
...
def index():
    ...
    if request.method == "POST":
        ...
        if q:
            try:
                res = requests.post(FASTAPI_URL, json={"question": q})
                data = res.json()
                a = data.get("answer", "")
                debug_log = data.get("log", [])
                conversation.append((q, a))
            except Exception as e:
                debug_log = [f"❌ Request to FastAPI failed: {e}"]
    return render_template_string(HTML_TEMPLATE, messages=conversation, log=debug_log)

# --- Run both backend and frontend ---
if __name__ == "__main__":
    print(f"🚀 Launching Mini LLM Server")
    print(f"🌐 Flask UI at     http://localhost:5000")
    print(f"🔌 FastAPI backend on http://127.0.0.1:{FASTAPI_PORT}/ask")
